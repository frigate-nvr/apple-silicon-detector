...
- **Standalone CLI binary**: The detector ships as an independent CLI tarball (`detector-macos-arm64.tar.gz`) in addition to the `.dmg`, enabling headless deployment without the GUI.
...
- **Start at login**: The CLI manages its own launchd plist (`com.frigate.apple-silicon-detector.plist`) for headless auto-start, while the GUI manages a separate plist (`com.frigate.apple-silicon-detector.gui.plist`) for login auto-start via `open -a`.
...
- **Model session reuse**: The detector skips redundant model reloads when Frigate re-requests the same model that is already loaded.
...
- **CI/CD pipeline**: GitHub Actions workflow with parallel jobs for CLI tarball and DMG builds, plus ruff lint/format and pytest.
- **Test suite**: Unit tests for `service_manager` and ZMQ smoke test script.
- **CLI Install from GUI**: The menu bar app can symlink the CLI tool to `~/.local/bin/detector` via the Advanced menu.
...
- Build pipeline rewritten: separate `build_cli.sh` and `build_app.sh` scripts, with the app build reusing the CLI build. Native Swift compilation + PyInstaller + codesign + DMG packaging.
- CI/CD runs CLI and App builds as parallel jobs for faster releases.
