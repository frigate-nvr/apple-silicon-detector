...
class ZmqIpcDetector(DetectionApi):
    ...
    def _create_socket(self) -> None:
        ...
        if endpoint.startswith("ipc://") and "../" in endpoint:
            import os
            ipc_path = endpoint[6:]  # Remove "ipc://" prefix
            current_dir = os.getcwd()
            resolved_path = os.path.normpath(os.path.join(current_dir, ipc_path))
            endpoint = f"ipc://{resolved_path}"
            logger.info(f"Resolved relative endpoint: {self._endpoint} -> {endpoint}")
        
        logger.info(f"ZMQ detector connecting to {endpoint}")
